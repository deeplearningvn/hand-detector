...
int main(int argc, char *argv[]) {
  ...
  while (cap.isOpened()) {
    ...
    cout << "Frame # " << iFrame << " FPS: " << fps << endl;
    ...
    outputs.clear();
    Status runStatus =
        session->Run({{inputLayer, tensor}}, outputLayer, {}, &outputs);
    if (!runStatus.ok()) {
    ...
    vector<size_t> goodIdxs =
        filterBoxes(scores, boxes, thresholdIOU, thresholdScore);
